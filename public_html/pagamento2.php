...
$atualizar_pag = new atualiza();
    $atualizar_pag->campo       = "boleto = '".str_replace("'","\'",str_replace('"','"',$resultado))."',status_transacao = ".$status_transacao.", cod_pagamento = ".$codpag.", data_gerado = '".date('Y-m-d H:i:s')."'";
    $atualizar_pag->tabela      = 'arpag_pagamento';
    $atualizar_pag->parametro   = "id = ".$id_pagamento;
    if (update_bd($atualizar_pag)) {
        echo "<script> carrega(); window.location='boleto.php?kj=".cript($id_pagamento)."';</script>"; 
        echo " <meta http-equiv='refresh' content=1;url='boleto.php?kj=".cript($id_pagamento)."'>";
    }
}