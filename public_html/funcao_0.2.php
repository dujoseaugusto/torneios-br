...
// Retorna true caso o  usuário esteja inadimplente 
function verif_inadimp($idusuario){
    $consulta = new consulta();
        $consulta->campo      = 'a.id';
        $consulta->parametro  = "DATE_ADD(a.data_gerado, INTERVAL 3 DAY) <= '".date('Y-m-d 00:00:00')."'
                                    AND a.status_transacao < 3 
                                    AND d.id = ".dcript($idusuario,'falha usuario sessao');
...
//verifica status do pagamento apartir do id da movimentação 
...
//verifica se o estatus de pagamento da incrição
...
function busca_modalidade(){
    $buscar_modalidade =  new consulta();
      ...
      $buscar_modalidade->parametro = 'nome';
    return  select_db_2($buscar_modalidade);
...
function retorna_status($id,$tabela){return $tabela;
                $consulta = new consulta();
