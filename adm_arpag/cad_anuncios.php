			...
			if(!preg_match("/^image\/(pjpeg|jpeg|png|gif|bmp)$/", $arquivo["type"])){
				...
				if ($up){
					$insertanu 	= new insercao();
								$insertanu->campo 	= 'nome,imagem,link';
								$insertanu->tabela 	= 'arpag_comercial';
								$insertanu->dados 	= "'".$nome."','".$imagem_nome."','".$link."'";
					if (insert_bd($insertanu))echo  utf8_encode("<script> alert ('Seu cadastro foi efetuado com sucesso !!!!');window.location='cad_anuncios.php';</script> <br /><br />");
...
<form id="form1" name="anuncios" method="post" action="cad_anuncios.php" enctype="multipart/form-data">
  ...
  <br /><br />
  <label>Link:</label><br />
  <input type="text"  name="link" id="link" /> (Ex.: www.site.com.br, não deve-se colocar "http://")
  <br /><br />
  <label>Foto:</label><br />
...
<br />
<b style="color:#FF0000 !important;">Obs.: A imagem deve conter o tamanho de 168px(largura) x 125px(altura). Também é aconselhável que seja revisto o tamanho final em KB/MB para que o site não fique lento.</b>
<br /><br />
...
if(($ex = isset($_GET['e']) ? dcript($_GET['e'],'Erro de indentificacao para a exclusao') : NULL) == 1){
    ...
    if($file   	= trocanome($_GET['f'])) unlink ("../anuncios/".$file);
    $deletar 	= new exclui();
    			$deletar->tabela	='arpag_comercial';
    			$deletar->parametro = "id = ".$di;
    if (delete_db($deletar)) echo utf8_encode("<script> alert ('Exclusao executada com sucesso');window.location='cad_anuncios.php';</script> ");
}
echo "<br />";
$listaanun	= new consulta();
				$listaanun->campo 		= 'nome,id, imagem';
				$listaanun->tabela		= 'arpag_comercial';
				$listaanun->parametro	= 'id DESC';
$listexect = select_db_2($listaanun);
while ($li = $listexect->fetch_array()) {
    echo "<tr><td>
    		<a href='cad_anuncios.php?d=".cript($li['id'])."&e=".cript(1)."&f=".$li['imagem']."'class='btn btn-danger'>Excluir</a>
    		 - ".$li['nome']."<br /></td></tr>";
}
?>
</table>
