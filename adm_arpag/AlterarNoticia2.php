...
//$dat        = $data . $h . $m . "00";
    ...
    if($arquivo["name"])
    {
        ...
        else
        {
            ...
            if ($up){
                ...
                $atualizanu  = new atualiza();
                  $atualizanu->campo      = "titulo = '".$titulo."', autor = '".$autor."', texto = '".$texto."', foto = '".$imagem_nome."', 
                                              subti = '".$sub."'";
                  $atualizanu->tabela     = 'arpag_noticia';
                  $atualizanu->parametro  = "id = ".$id;
                  if (insert_bd($atualizanu)){
      ...
      $atualizanu  = new atualiza();
                  $atualizanu->campo      = "titulo = '".$titulo."', autor = '".$autor."', texto = '".$texto."', subti = '".$sub."'";
                  $atualizanu->tabela     = 'arpag_noticia';
                  $atualizanu->parametro  = "id = ".$id;
                  if (update_bd($atualizanu)){
                    echo "<p class='alert-danger'>Seu cadastro foi efetuado com sucesso !!!! <br /><br />
                            Clique <a href='AlterarNoticia.php'>aqui</a> para voltar</p>";
                  //exit;
                }else echo mysql_error();
...
?>
<h3>Alterar Not&iacute;cias</h3>
<br />
<?php
if($noticia =  isset($_GET['d']) ? dcript($_GET['d'],'Erro ao abrir noticia') : NULL){
  $s_noticia = new consulta();
    $s_noticia->campo     = 'id,titulo,autor,texto,foto,subti,video,idgaleria,data';
    $s_noticia->tabela    = 'arpag_noticia';
    $s_noticia->parametro = "id = ".$noticia;
...
?>
      <form id="form1" name="form1" method="post" action="AlterarNoticia2.php" enctype="multipart/form-data" onSubmit="return validaForm()">
          ...
          <label>Data:</label><br />
          <input type="text" value="<?php echo $linha_not['data']; ?>" name="data" class="data calendario" onkeyup="formata('data');" />
          ...
          <input name="subtitulo" type="text" id="subtitulo" value="<?php echo utf8_encode($linha_not['subti']);?>"  />

          <br />

          <label>Autor:</label><br />
          <input name="autor" type="text" id="autor" size="45" maxlength ="60" value="<?php echo utf8_encode($linha_not['autor']);?>" />
         ...
         <!-- <br /><br />
          ...
          <select name="galeria" id="galeria">
              ...
              $consulta = new consulta();
                ...
                $consulta->tabela     = "arpag_galeria";
                $consulta->parametro  = "nome";
              $executa = select_db_2($consulta);
              while ($lin =  $executa->fetch_array()){
                echo "<option value='".$lin[id]."'>".utf8_encode($lin[nome])."</option>";
              }
            ...
            $sr = mysql_query($sql);
                while ($lin = mysql_fetch_row($sr)){
                  echo utf8_encode("<option value=\"$lin[0]\">$lin[1]</option>");
                }
