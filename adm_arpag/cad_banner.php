...
<?php
  ...
  $nome = $_POST["nome"];
	...
	if (isset($nome) ? trocanome($nome) : null){
		$erro    = $config = array();
			...
			if(!preg_match("/^image\/(pjpeg|jpeg|png|gif|bmp)$/", $arquivo["type"])){
				...
				if ($up){
					$insertanu 	= new insercao();
								...
								$insertanu->tabela 	= 'arpag_banner';
								$insertanu->dados 	= "'".$nome."','".$imagem_nome."'";
					if (insert_bd($insertanu))echo  utf8_encode("<script> alert ('Seu cadastro foi efetuado com sucesso !!!!');window.location='cad_banner.php';</script> <br /><br />");
...
<h3>Cadastro de Banner (Background Inicial)</h3> <br /><br />
<form id="form1" name="banner" method="post" action="cad_banner.php" enctype="multipart/form-data">
...
<br />
<b style="color:#FF0000 !important;">Obs.: A imagem deve conter o tamanho de 1450px(largura) x 141px(altura). Também é aconselhável que seja revisto o tamanho final em KB/MB para que o site não fique lento.</b>
<br /><br />
<h3>Banners cadastrados (Inicial)</h3>
<table class='table'>
<?php
if(($ex = isset($_GET['e']) ? dcript($_GET['e'],'Erro de indentificacao para a exclusao') : NULL) == 1){
    ...
    if($file   	= trocanome($_GET['f'])) unlink ("../banner/".$file);
    $deletar 	= new exclui();
    			$deletar->tabela	='arpag_banner';
    			$deletar->parametro = "id = ".$di;
    if (delete_db($deletar)) echo utf8_encode("<script> alert ('Exclusao executada com sucesso');window.location='cad_banner.php';</script> ");
}
echo "<br />";
$listaanun	= new consulta();
				$listaanun->campo 		= 'nome,id,imagem';
				$listaanun->tabela		= 'arpag_banner';
				$listaanun->parametro	= 'id DESC';
$listexect = select_db_2($listaanun);
while ($li = $listexect->fetch_array()) {
    echo "<tr><td>
    		<a href='cad_banner.php?d=".cript($li['id'])."&e=".cript(1)."&f=".$li['imagem']."'class='btn btn-danger'>Excluir</a>
    		 - ".$li['nome']."<br /></td></tr>";
}
?>
</table>
